...
document.body.append(
  createTag(
    "div",
    { id: "Doc_Nations" },
    (divTag) => divTag.style.setProperty("display", "none"),
  ),
);
...
formTag.parentElement?.insertBefore(
  createTag("div", (divTag) =>
    divTag.append(
      createTag("label", {
        htmlFor: "Doc_Score",
        textContent: "Nation Score:",
      }),
      createTag("input", {
        id: "Doc_Score",
        type: "number",
        value: localStorage.getItem(scoreKey) ?? "0",
      }, (inputTag) =>
        inputTag.addEventListener("change", function (_event): void {
      ...
      createTag("button", {
        ...
        buttonTag.addEventListener(
          "click",
          async function (_event): Promise<void> {
            this.disabled = true;
            const score = parseFloat(
              new DOMParser()
                .parseFromString(
                  await (await fetch("https://politicsandwar.com/nation/war/"))
                    .text(),
                  "text/html",
                )
            ...
            (this.previousElementSibling!
              .previousElementSibling as HTMLInputElement).valueAsNumber =
                score;
            this.disabled = false;
          },
        );
...
/* Functions
-------------------------*/
function inSpyRange(score: number, rangeScore: number): boolean {
  return rangeScore * 0.4 <= score && score <= rangeScore * 2.5;
}

function inWarRange(score: number, rangeScore: number): boolean {
  return rangeScore * 0.75 <= score && score <= rangeScore * 1.75;
}

function updateIcons(myScore: number): void {
  [...document.querySelectorAll<HTMLTableRowElement>(
    ".nationtable tr",
  )].slice(1).forEach((trTag) => {
    const tdTag = trTag.lastElementChild as HTMLTableCellElement;
    const theirScore = parseFloat(
      tdTag.lastChild!.textContent!.replaceAll(",", ""),
    );
    [...tdTag.querySelectorAll<HTMLImageElement>("img")].forEach((imgTag) => {
      if (
        imgTag.src === "https://politicsandwar.com/img/icons/16/plus_shield.png"
      ) {
        return;
      }
      imgTag.remove();
    ...
    if (inSpyRange(theirScore, myScore)) {
      tdTag.insertBefore(
        createTag("img", {
          src: "https://politicsandwar.com/img/icons/16/emotion_spy.png",
        }),
        tdTag.lastChild,
      );
    }
    if (inWarRange(theirScore, myScore)) {
      if (inWarRange(myScore, theirScore)) {
        tdTag.insertBefore(
          createTag("img", {
            src: "https://docscripts.stagintin.com/icons/green_red.png",
          }),
          tdTag.lastChild,
        );
      } else {
        tdTag.insertBefore(
          createTag("img", {
            src: "https://docscripts.stagintin.com/icons/green.png",
          }),
          tdTag.lastChild,
        );
      }
      ...
      tdTag.insertBefore(
        createTag("img", {
          src: "https://docscripts.stagintin.com/icons/red.png",
        }),
        tdTag.lastChild,
      );
    }
