...
document.querySelectorAll('form[action*="#improvements"]')
  .forEach((formTag) =>
    (formTag as HTMLFormElement)
      .addEventListener("click", async function (event): Promise<void> {
        if (
          !event.target ||
        ...
        dom.querySelectorAll(".improvementQuantity").forEach((
          spanTag,
          i,
        ) =>
          spanTags[i].textContent = spanTag.textContent
        );
        (document.querySelector(
          "#improvements",
        ) as HTMLParagraphElement).insertBefore(
          dom.querySelector("#improvements>span") as HTMLSpanElement,
          pass(
            document.querySelector(
              "#improvements>span",
            ) as HTMLSpanElement,
