				...
				(buttonTag) =>
					buttonTag.addEventListener('click', async (event) => {
						...
						const score = parseFloat(
							...
							) as HTMLAnchorElement).href.split('?')[1].split(
								'&',
							).find((arg) => arg.startsWith('keyword='))?.slice(
								8,
							) ?? '0',
						)
						...
						if (buttonTag.previousSibling) {
							buttonTag.previousSibling.textContent = date
								.toJSON()
						}
						localStorage.setItem(ticksKey, `${date.getTime()}`)
						localStorage.setItem(scoreKey, `${score}`)
...
function updateIcons(myScore: number) {
	;[...document.querySelectorAll(
		'.nationtable tr',
	) as unknown as HTMLTableRowElement[]].slice(1).forEach((trTag) => {
		const tdTag = trTag.lastElementChild as HTMLTableCellElement
		const theirScore = parseFloat(
			tdTag.lastChild?.textContent?.replaceAll(',', '') as string,
		)
		;[...tdTag.querySelectorAll('img') as unknown as HTMLImageElement[]]
			.forEach((imgTag) => {
				if (
					imgTag.src ===
						'https://politicsandwar.com/img/icons/16/plus_shield.png'
				) {
					return
				}
				imgTag.remove()
		...
		if (inSpyRange(theirScore, myScore)) {
			tdTag.insertBefore(
				build(
					<img src='https://politicsandwar.com/img/icons/16/emotion_spy.png' />,
				),
				tdTag.lastChild,
			)
		}
		if (inWarRange(theirScore, myScore)) {
			if (inWarRange(myScore, theirScore)) {
				tdTag.insertBefore(
					build(
						<img src='https://docscripts.stagintin.com/icons/green_red.png' />,
					),
					tdTag.lastChild,
				)
			} else {
				tdTag.insertBefore(
					build(
						<img src='https://docscripts.stagintin.com/icons/green.png' />,
					),
					tdTag.lastChild,
				)
			}
			...
			tdTag.insertBefore(
				build(
					<img src='https://docscripts.stagintin.com/icons/red.png' />,
				),
				tdTag.lastChild,
			)
		}
