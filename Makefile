release:
	SERVER_FN_OVERRIDE_KEY="./trade" cargo build --bin trade --release --target=wasm32-unknown-unknown
	mkdir -p static/wasm/
...
debug:
	SERVER_FN_OVERRIDE_KEY="./trade" cargo build --bin trade --target=wasm32-unknown-unknown
	mkdir -p static/wasm/
